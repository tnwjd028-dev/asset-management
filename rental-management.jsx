...
import { Bell, Plus, Edit2, Trash2, Search, Send, X, Mail, CreditCard, ChevronUp, ChevronDown, Paperclip, Download, FileText, Upload, RotateCcw, Clock, ChevronRight, User, Users } from "lucide-react";
...
const EMPTY = {
  ...
  requester:"", requesterRank:"", requesterDept:"",
  borrower:"",  borrowerRank:"",  department:"",  contact:"",
  purpose:"",   loanDate:"",      dueDate:"",     notes:"",
  securityFileId:null, extensions:[], completedDate:null,
};
...
const ST_ALL  = ["전체","사용중","기한임박/만료임박","기한지연","기간만료"];
const ST_CARD = ["전체","사용중","만료임박","기간만료"];
const ST_MAIL = ["전체","사용중","기한임박","기한지연"];
...
export default function App() {
  ...
  useEffect(()=>{
    async function load(){
      try {
        const r  = await window.storage.get("rental_v8");
        const rf = await window.storage.get("rental_files_v8");
        setItems(r?.value ? JSON.parse(r.value) : SAMPLES);
        if (rf?.value) setFileMap(JSON.parse(rf.value));
      } catch { setItems(SAMPLES); }
      setLoaded(true);
    }
    load();
  },[]);
  useEffect(()=>{ if(loaded) window.storage.set("rental_v8",JSON.stringify(items)).catch(()=>{}); },[items,loaded]);
  useEffect(()=>{ if(loaded) window.storage.set("rental_files_v8",JSON.stringify(fileMap)).catch(()=>{}); },[fileMap,loaded]);
  ...
  const filtered = useMemo(()=>{
    ...
    return [...r].sort((a,b)=>{
      ...
      if(sortCol==="status"){ const o=["overdue","expired","due_soon","expiring_soon","active"]; va=o.indexOf(a.status); vb=o.indexOf(b.status); }
  ...
  function save(){
    ...
    showToast(editId?"수정되었습니다.":"등록되었습니다."); setModal(null);
  }
  function del(id){ setItems(p=>p.filter(i=>i.id!==id)); showToast("삭제되었습니다.","err"); }
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:C.bgTer,minHeight:"100vh",padding:"1.5rem"}}>
              ...
              <tbody>
                {filtered.length===0
                  ...
                  : filtered.map((item,idx)=>{
                    ...
                    const rowBg=idx%2===1?C.bgSub:C.bg;
                    ...
                    return (
                      <tr key={item.id} style={{borderBottom:`1px solid ${C.border}`,background:rowBg,cursor:"pointer"}}
                        ...
                        {/* 상태 */}
                        <td style={{padding:"10px 14px"}}><span style={pill(item.status)}>{ST[item.status].label}</span></td>
                        {/* 액션 */}
                        <td style={{padding:"8px 10px"}}>
                          <div style={{display:"flex",gap:4,justifyContent:"flex-end"}} onClick={e=>e.stopPropagation()}>
                            {isMailNoti&&<button onClick={()=>{setNotifItem(item);setModal("single");}} title="알림" style={iBtn(C.warning)}><Bell size={13}/></button>}
                            {item.category==="명함"&&item.securityFileId&&fileMap[item.securityFileId]&&(
      ...
      {modal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50,padding:"1rem"}}
          ...
          {/* 엑셀 양식 안내 */}
          {modal==="xlsxGuide"&&(
            <div style={{background:C.bg,borderRadius:16,border:`1px solid ${C.border}`,width:"100%",maxWidth:700,maxHeight:"88vh",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"1.25rem 1.5rem",borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
                <div>
                  ...
                  <h3 style={{margin:0,fontSize:16,fontWeight:700,color:C.text}}>엑셀 가져오기 양식 안내</h3>
                </div>
                <button onClick={()=>setModal(null)} style={iBtn()}><X size={14}/></button>
              </div>
              ...
              <div style={{overflowY:"auto",flex:1,padding:"1.25rem 1.5rem"}}>
                ...
                {/* 안내 문구 */}
                <div style={{padding:"10px 14px",borderRadius:8,background:C.infoBg,border:`1px solid ${C.infoBorder}`,fontSize:13,color:C.info,marginBottom:"1.25rem",lineHeight:1.6}}>
                  아래 양식에 맞춰 엑셀 파일을 작성한 뒤 업로드해 주세요. 1행은 헤더(컬럼명), 2행부터 데이터를 입력합니다.<br/>
                  <strong>메일주소</strong>는 아이디만 입력하거나 전체 주소 모두 사용 가능합니다 — 도메인 없는 경우 <strong>{DOMAIN}</strong>이 자동으로 붙습니다.
                </div>

                {/* 컬럼 구조 */}
                <div style={{fontSize:12,fontWeight:700,color:C.textSub,marginBottom:8,letterSpacing:"0.04em"}}>컬럼 구성 (9개)</div>
                <div style={{borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden",marginBottom:"1.25rem"}}>
                  {/* 그룹 헤더 */}
                  <div style={{display:"grid",gridTemplateColumns:"3fr 3fr 1fr 1fr 1fr",background:C.accent,color:"#fff",fontSize:11,fontWeight:700,letterSpacing:"0.04em"}}>
                    <div style={{padding:"8px 12px",borderRight:`1px solid rgba(255,255,255,0.2)`}}>신청인 정보 (A~C)</div>
                    <div style={{padding:"8px 12px",borderRight:`1px solid rgba(255,255,255,0.2)`}}>사용인 정보 (D~F)</div>
                    <div style={{padding:"8px 12px",borderRight:`1px solid rgba(255,255,255,0.2)`}}>사용 시작일 (G)</div>
                    <div style={{padding:"8px 12px",borderRight:`1px solid rgba(255,255,255,0.2)`}}>사용 종료일 (H)</div>
                    <div style={{padding:"8px 12px"}}>사용처 (I)</div>
                  </div>
                  {/* 세부 컬럼명 */}
                  <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)",borderBottom:`1px solid ${C.border}`,background:C.bgSub}}>
                    {[
                      {col:"A",name:"이름",group:"req"},
                      {col:"B",name:"직급",group:"req"},
                      {col:"C",name:"부서",group:"req"},
                      {col:"D",name:"이름",group:"use"},
                      {col:"E",name:"직급",group:"use"},
                      {col:"F",name:"메일주소",group:"use"},
                      {col:"G",name:"사용 시작일",group:"date"},
                      {col:"H",name:"사용 종료일",group:"date"},
                      {col:"I",name:"사용처",group:"etc"},
                    ].map(({col,name,group})=>(
                      <div key={col} style={{padding:"8px 10px",borderRight:col!=="I"?`1px solid ${C.border}`:"none",borderBottom:`1px solid ${C.border}`}}>
                        <div style={{fontSize:10,color:C.textTer,fontWeight:600,marginBottom:2}}>{col}</div>
                        <div style={{fontSize:12,fontWeight:600,color:C.text}}>{name}</div>
                      </div>
                    ))}
                  </div>
                  {/* 샘플 행 */}
                  <div style={{background:C.bgSub}}>
                    <div style={{padding:"6px 12px 4px",fontSize:10,fontWeight:700,color:C.textTer,letterSpacing:"0.04em"}}>작성 예시</div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)"}}>
                      {[
                        "정도현","대리","총무팀",
                        "최유나","주임","vendor01",
                        "2026-05-01","2026-07-31","UI 외주 프로젝트"
                      ].map((v,i)=>(
                        <div key={i} style={{padding:"6px 10px",borderRight:i<8?`1px solid ${C.border}`:"none",fontSize:12,color:i<3?"#1e40af":i<6?"#166534":C.textSub,fontWeight:i===5?600:400}}>
                          {v}
                        </div>
                      ))}
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)",borderTop:`1px solid ${C.border}`}}>
                      {[
                        "이관우","과장","인사팀",
                        "이수진","사원","vendor02@bimatrix.co.kr",
                        "2026-05-10","2026-08-31","채용 프로세스 운영"
                      ].map((v,i)=>(
                        <div key={i} style={{padding:"6px 10px",borderRight:i<8?`1px solid ${C.border}`:"none",fontSize:12,color:i<3?"#1e40af":i<6?"#166534":C.textSub,fontWeight:i===5?600:400}}>
                          {v}
                        </div>
                ...
                {/* 날짜 형식 안내 */}
                <div style={{fontSize:12,fontWeight:700,color:C.textSub,marginBottom:8,letterSpacing:"0.04em"}}>날짜 형식 (모두 인식 가능)</div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:"1.25rem"}}>
                  {["2026-05-01","2026.05.01","2026/05/01","2026년 5월 1일","엑셀 날짜 셀"].map(v=>(
                    <span key={v} style={{padding:"4px 10px",borderRadius:6,border:`1px solid ${C.border}`,background:C.bgSub,fontSize:12,color:C.textSub,fontFamily:"monospace"}}>{v}</span>
                  ))}
                </div>

                {/* 주의사항 */}
                <div style={{padding:"10px 14px",borderRadius:8,background:C.warningBg,border:`1px solid ${C.warningBorder}`,fontSize:12,color:C.warning,lineHeight:1.8}}>
                  <strong>주의사항</strong><br/>
                  · 1행은 반드시 헤더(컬럼명)로 작성해 주세요.<br/>
                  · 빈 행은 자동으로 건너뜁니다.<br/>
                  · 가져오기 후 개별 수정이 가능합니다.
                </div>
              </div>

              <div style={{padding:"1rem 1.5rem",borderTop:`1px solid ${C.border}`,background:C.bgSub,display:"flex",gap:8,justifyContent:"space-between",alignItems:"center",flexShrink:0,flexWrap:"wrap"}}>
                <button onClick={async ()=>{
                  try {
                    const XLSX=await import("https://esm.sh/xlsx@0.20.1");
                    const headers=["이름(신청인)","직급(신청인)","부서(신청인)","이름(사용인)","직급(사용인)","메일주소(아이디)","사용 시작일","사용 종료일","사용처"];
                    const sample1=["정도현","대리","총무팀","최유나","주임","vendor01","2026-05-01","2026-07-31","UI 외주 프로젝트"];
                    const sample2=["이관우","과장","인사팀","이수진","사원","vendor02","2026-05-10","2026-08-31","채용 프로세스 운영"];
                ...
                }} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"0 16px",height:36,borderRadius:8,border:`1px solid ${C.border}`,background:C.bg,color:C.textSub,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>
                  <Download size={14}/> 양식 파일 다운로드
                </button>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>setModal(null)} style={{display:"inline-flex",alignItems:"center",padding:"0 16px",height:36,borderRadius:8,border:`1px solid ${C.border}`,background:"transparent",color:C.textSub,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>닫기</button>
                  <button onClick={()=>xlsxRef.current?.click()} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"0 20px",height:36,borderRadius:8,border:"none",background:C.accent,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>
          ...
          {modal==="notif"&&(
            <div style={{background:C.bg,borderRadius:16,border:`1px solid ${C.border}`,width:"100%",maxWidth:480,padding:"1.5rem",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
              ...
              <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                <button onClick={()=>setModal(null)} style={{display:"inline-flex",alignItems:"center",padding:"0 16px",height:36,borderRadius:8,border:`1px solid ${C.border}`,background:"transparent",color:C.textSub,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>취소</button>
                <button onClick={()=>{setModal(null);showToast(`${mailOverdue.length}건 알림을 발송했습니다.`);}} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"0 20px",height:36,borderRadius:8,border:"none",background:C.danger,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>
                  <Send size={13}/> 발송하기
                </button>
              </div>
            </div>
          ...
          {modal==="single"&&notifItem&&(
            <div style={{background:C.bg,borderRadius:16,border:`1px solid ${C.border}`,width:"100%",maxWidth:440,padding:"1.5rem",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
              ...
              <textarea defaultValue={`안녕하세요 ${notifItem.borrower}님,\n사용 중이신 메일계정 [${notifItem.itemName}]의 사용 기한 (${fmtDate(notifItem.dueDate)})이 ${notifItem.status==="overdue"?"지났습니다. 계정 비활성화 처리가 필요하오니 확인 부탁드립니다.":"곧 도래합니다. 연장 또는 종료 여부를 알려 주시기 바랍니다."}\n감사합니다.`}
                rows={5} style={{...inp(),height:"auto",padding:"10px",resize:"vertical",lineHeight:1.7}}/>
              <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}>
                <button onClick={()=>setModal(null)} style={{display:"inline-flex",alignItems:"center",padding:"0 16px",height:36,borderRadius:8,border:`1px solid ${C.border}`,background:"transparent",color:C.textSub,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>취소</button>
                <button onClick={()=>{setModal(null);showToast(`${notifItem.borrower}님께 알림을 발송했습니다.`);}} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"0 20px",height:36,borderRadius:8,border:"none",background:C.accent,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>
                  <Send size={13}/> 발송하기
                </button>
              </div>
            </div>
