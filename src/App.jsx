          ...
          {modal==="notif"&&(
            <div style={{background:C.bg,borderRadius:16,border:`1px solid ${C.border}`,width:"100%",maxWidth:480,padding:"1.5rem",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
              ...
              <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                ...
                <button onClick={async()=>{
                  setModal(null);
                  const result=await sendBulkNotifications(mailOverdue,(item)=>`안녕하세요 ${item.borrower}님,\n사용 중이신 메일계정 [${item.itemName}]의 사용 기한 (${fmtDate(item.dueDate)})이 지났습니다.\n계정 비활성화 처리가 필요하오니 확인 부탁드립니다.\n감사합니다.`);
